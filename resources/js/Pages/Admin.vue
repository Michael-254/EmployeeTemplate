  ...
  <div class="min-h-screen font-mono">
    <section class="max-w-5xl mx-auto">
      <div class="border rounded-md shadow-md mt-4 bg-white px-3">
          <h3 class="text-center text-xl text-blue-700 font-bold font-sans m-2">Employees</h3>

          <div class="flex float-center mb-2 space-x-2">
               <select v-model="filterId" type="text" class="w-full py-2 px-2 bg-gray-200 rounded-lg shadow-sm focus:outline-none
                  focus:shadow-outline text-gray-600">
                   <option value=""> -- Filter By Name -- </option>
                   <option v-for="filter in filters" :key="filter.id" :value="filter.id"> {{filter.name}} </option>
               </select>
               <select  v-model="dept" class="w-full py-2 px-2 bg-gray-200 rounded-lg shadow-sm focus:outline-none
                  focus:shadow-outline text-gray-600">
                ...
                <tbody v-for="user in data.data" :key="user.id">
                  <tr class="hover:bg-grey-lighter">
                    <td class="py-4 px-6 border-b border-grey-light">
                      <span class="font-bold badge badge-success">
                         Name:
                      </span> 
                      {{user.name}}
                      </td>
                    ...
                    <td class="py-4 px-6 border-b border-grey-light">
                      <span class="badge badge-warning font-bold cursor-pointer">
                         <router-link :to="{ name: 'admin.view', params: { id: user.id}}">View Details</router-link>
                     </span>
                    </td>
                  </tr>
                </tbody>
...
export default {
  ...
  watch: {
     filterId(value) {this.getResults();},
     dept(value) {this.getResults();}
  },
  methods: {
       getResults(page = 1){
                axios.get('/Admin?page=' + page + '&filterId=' + this.filterId + '&dept=' + this.dept)
                .then(response => {
